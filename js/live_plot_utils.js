...
function fileToPlotlyDataObj(file,dataObjs) {
        ...
        fs.stat(file, function(err, stat) {
            if(err == null) {
                fs.readFile(file, 'utf8', function (err,dataS) {
                    ...
                    console.log('plotly file read is successful ' + file);
                    obj.data = transpose(obj.data);
//                    obj.data.map((_, colIndex) => obj.data.map(row => row[colIndex]));
                    console.log(obj);
                    dataObjs.push(obj);
                    resolve('plotly file read success!');
                });
            }// end null
            else{ // always resolve ...
                dataObjs.push(obj);
                console.log('plotly file read failed ' + file);
                console.log(err);
                resolve('plotly file read failed!');
